...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas) {
      ...
      // Fallback to basic formats if advanced formats are not supported
      if (!formatRGBA) {
    ...
    function initFramebuffers() {
      if (!ext) {
        console.warn('WebGL extensions not available');
        return;
      }
    ...
    function scaleByPixelRatio(input) {
      const pixelRatio = Math.min(1.5, window.devicePixelRatio || 1); // cap DPR to reduce GPU load
